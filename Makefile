# target: all - Default target. Compile templates, minify JavaScript, and minify CSS.
all: clean build-templates build-js build-css
...
build-templates:
	handlebars templates -f gen/templates.js -k if -k unless -k with -k round -k roundWinp -k matchup -k face -k new_window -k skillsBlock -k currency -k numberWithCommas -k playerNameLabels -o
...
.PHONY: all check docs lint build-css build-js build-templates clean