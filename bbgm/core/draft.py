...
def generate_players():
    ...
    roster_order = 1
    r = g.dbex('SELECT pr.pid FROM player_attributes as pa, player_ratings as pr WHERE pa.pid = pr.pid AND pa.tid = :tid AND pr.season = :season ORDER BY pr.overall + 2*pr.pot DESC', tid=c.PLAYER_UNDRAFTED, season=g.season)
...
def until_user_or_end():
    ...
    for tid, draft_round, pick in r.fetchall():
        ...
        r = g.dbex('SELECT pr.pid FROM player_attributes as pa, player_ratings as pr WHERE pa.pid = pr.pid AND pa.tid = :tid AND pr.season = :season ORDER BY pr.overall + 2*pr.pot DESC LIMIT :pick, 1', tid=c.PLAYER_UNDRAFTED, season=g.season, pick=team_pick)
        pid,= r.fetchone()
        pick_player(tid, pid)
        pids.append(pid)
...
def pick_player(tid, pid):
    ...
    r = g.dbex('SELECT pa.name, pa.pos, pa.born_year, pr.overall, pr.pot FROM player_attributes AS pa, player_ratings AS pr WHERE pa.pid = pr.pid AND pa.tid = :tid AND pr.pid = :pid AND pr.season = :season', tid=c.PLAYER_UNDRAFTED, pid=pid, season=g.season)
    name, pos, born_year, overall, pot = r.fetchone()
    ...
    g.dbex('UPDATE draft_results SET pid = :pid, name = :name, pos = :pos, born_year = :born_year, overall = :overall, pot = :pot WHERE season = :season AND draft_round = :draft_round AND pick = :pick', pid=pid, name=name, pos=pos, born_year=born_year, overall=overall, pot=pot, season=g.season, draft_round=draft_round, pick=pick)
