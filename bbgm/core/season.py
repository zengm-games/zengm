...
def awards():
    ...
    # Cache averages
    g.dbex('CREATE TEMPORARY TABLE awards_avg (pid INTEGER PRIMARY KEY, name VARCHAR(255), tid INTEGER, abbrev VARCHAR(3), draft_year INTEGER, games_played INTEGER, games_started INTEGER, mpg FLOAT, ppg FLOAT, rpg FLOAT, apg FLOAT, bpg FLOAT, spg FLOAT)')
    g.dbex('INSERT INTO awards_avg (pid, name, tid, abbrev, draft_year, games_played, games_started, mpg, ppg, rpg, apg, bpg, spg) (SELECT pa.pid, pa.name, pa.tid, ta.abbrev, pa.draft_year, SUM(ps.min>0) AS games_played, SUM(ps.gs) AS games_started, AVG(ps.min) AS mpg, AVG(ps.pts) AS ppg, AVG(ps.orb+ps.drb) AS rpg, AVG(ps.ast) AS apg, AVG(ps.blk) AS bpg, AVG(ps.stl) AS spg FROM player_attributes as pa, player_stats as ps, team_attributes as ta WHERE pa.pid = ps.pid AND ps.season = :season AND ps.playoffs = 0 AND ta.tid = pa.tid AND ta.season = ps.season GROUP BY ps.pid)', season=g.season)
    ...
    r = g.dbex('SELECT pid, name, tid, abbrev, ppg, rpg, apg FROM awards_avg ORDER BY (0.75 * ppg) + apg + rpg DESC')
    mvp =  r.fetchone()
    r = g.dbex('SELECT pid, name, tid, abbrev, rpg, bpg, spg FROM awards_avg ORDER BY rpg + 5 * bpg + 5 * spg DESC')
    dpoy = r.fetchone()
    r = g.dbex('SELECT pid, name, tid, abbrev, ppg, rpg, apg FROM awards_avg WHERE games_played/(games_started+1) > 2 ORDER BY (0.75 * ppg) + apg + rpg DESC')
    smoy = r.fetchone()
    r = g.dbex('SELECT pid, name, tid, abbrev, ppg, rpg, apg FROM awards_avg WHERE draft_year = :season - 1 ORDER BY (0.75 * ppg) + apg + rpg DESC', season=g.season)
    roy = r.fetchone()

    g.dbex('INSERT INTO awards (season, bre_tid, bre_abbrev, bre_region, bre_name, bre_won, bre_lost, brw_tid, brw_abbrev, brw_region, brw_name, brw_won, brw_lost, mvp_pid, mvp_name, mvp_tid, mvp_abbrev, mvp_ppg, mvp_rpg, mvp_apg, dpoy_pid, dpoy_name, dpoy_tid, dpoy_abbrev, dpoy_rpg, dpoy_bpg, dpoy_spg, smoy_pid, smoy_name, smoy_tid, smoy_abbrev, smoy_ppg, smoy_rpg, smoy_apg, roy_pid, roy_name, roy_tid, roy_abbrev, roy_ppg, roy_rpg, roy_apg) VALUES (:season, :bre_tid, :bre_abbrev, :bre_region, :bre_name, :bre_won, :bre_lost, :brw_tid, :brw_abbrev, :brw_region, :brw_name, :brw_won, :brw_lost, :mvp_pid, :mvp_name, :mvp_tid, :mvp_abbrev, :mvp_ppg, :mvp_rpg, :mvp_apg, :dpoy_pid, :dpoy_name, :dpoy_tid, :dpoy_abbrev, :dpoy_rpg, :dpoy_bpg, :dpoy_spg, :smoy_pid, :smoy_name, :smoy_tid, :smoy_abbrev, :smoy_ppg, :smoy_rpg, :smoy_apg, :roy_pid, :roy_name, :roy_tid, :roy_abbrev, :roy_ppg, :roy_rpg, :roy_apg)', season=g.season, bre_tid=bre['tid'], bre_abbrev=bre['abbrev'], bre_region=bre['region'], bre_name=bre['name'], bre_won=bre['won'], bre_lost=bre['lost'], brw_tid=brw['tid'], brw_abbrev=brw['abbrev'], brw_region=brw['region'], brw_name=brw['name'], brw_won=brw['won'], brw_lost=brw['lost'], mvp_pid=mvp['pid'], mvp_name=mvp['name'], mvp_tid=mvp['tid'], mvp_abbrev=mvp['abbrev'], mvp_ppg=mvp['ppg'], mvp_rpg=mvp['rpg'], mvp_apg=mvp['apg'], dpoy_pid=dpoy['pid'], dpoy_name=dpoy['name'], dpoy_tid=dpoy['tid'], dpoy_abbrev=dpoy['abbrev'], dpoy_rpg=dpoy['rpg'], dpoy_bpg=dpoy['bpg'], dpoy_spg=dpoy['spg'], smoy_pid=smoy['pid'], smoy_name=smoy['name'], smoy_tid=smoy['tid'], smoy_abbrev=smoy['abbrev'], smoy_ppg=smoy['ppg'], smoy_rpg=smoy['rpg'], smoy_apg=smoy['apg'], roy_pid=roy['pid'], roy_name=roy['name'], roy_tid=roy['tid'], roy_abbrev=roy['abbrev'], roy_ppg=roy['ppg'], roy_rpg=roy['rpg'], roy_apg=roy['apg'])

    g.dbex('INSERT INTO awards_all_league (season, team_type, pid, name, abbrev, ppg, rpg, apg, bpg, spg) (SELECT :season, \'league\', pid, name, abbrev, ppg, rpg, apg, bpg, spg FROM awards_avg ORDER BY (0.75 * ppg) + apg + rpg DESC LIMIT 15)', season=g.season)
    g.dbex('INSERT INTO awards_all_league (season, team_type, pid, name, abbrev, ppg, rpg, apg, bpg, spg) (SELECT :season, \'defensive\', pid, name, abbrev, ppg, rpg, apg, bpg, spg FROM awards_avg ORDER BY rpg + 5 * bpg + 5 * spg DESC LIMIT 15)', season=g.season)
