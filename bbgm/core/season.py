...
def new_phase(phase):
    ...
    if phase == c.PHASE_PRESEASON:
        ...
        r = g.dbex('SELECT pr.pid, season + 1 AS season, overall, pr.height, strength, speed, jumping, endurance, shooting_inside, shooting_layups, shooting_free_throws, shooting_two_pointers, shooting_three_pointers, blocks, steals, dribbling, passing, rebounding, potential FROM player_ratings AS pr, player_attributes AS pa WHERE pa.pid = pr.pid AND pr.season = :season AND pa.tid != :tid', season=g.season - 1, tid=c.PLAYER_RETIRED)
        for row in r.fetchall():
            g.dbex('INSERT INTO player_ratings (pid, season, overall, height, strength, speed, jumping, endurance, shooting_inside, shooting_layups, shooting_free_throws, shooting_two_pointers, shooting_three_pointers, blocks, steals, dribbling, passing, rebounding, potential) VALUES (:pid, :season, :overall, :height, :strength, :speed, :jumping, :endurance, :shooting_inside, :shooting_layups, :shooting_free_throws, :shooting_two_pointers, :shooting_three_pointers, :blocks, :steals, :dribbling, :passing, :rebounding, :potential)', **row)
...
def new_schedule_playoffs_day():
    ...
    if not active_series:
        ...
        # Who won?
        winners = {}
        r = g.dbex('SELECT series_id, tid_home, tid_away, seed_home, seed_away, won_home, won_away FROM playoff_series WHERE series_round = :series_round AND season = :season ORDER BY series_id ASC', series_round=current_round, season=g.season)
        for row in r.fetchall():
            series_id, tid_home, tid_away, seed_home, seed_away, won_home, won_away = row
            if won_home == 4:
                winners[series_id] = [tid_home, seed_home]
            else:
                winners[series_id] = [tid_away, seed_away]
            # Record user's team as conference and league champion
            if current_round == 3:
                g.dbex('UPDATE team_attributes SET won_conference = 1 WHERE season = :season AND tid = :tid', season=g.season, tid=winners[series_id][0])
            elif current_round == 4:
                g.dbex('UPDATE team_attributes SET won_championship = 1 WHERE season = :season AND tid = :tid', season=g.season, tid=winners[series_id][0])
