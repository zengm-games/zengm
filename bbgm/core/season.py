...
def awards():
    ...
    # Cache averages
    g.db.execute('CREATE TEMPORARY TABLE %s_awards_avg (name TEXT, abbreviation TEXT, draft_year INTEGER, born_location TEXT, games INTEGER, starter INTEGER, minutes FLOAT, points FLOAT, rebounds FLOAT, assists FLOAT, blocks FLOAT, steals FLOAT)', (g.league_id,))
    for player_id, in g.db.execute('SELECT player_id FROM player_attributes WHERE team_id >= 0'):
        g.db.execute('INSERT INTO awards_avg (name, abbreviation, draft_year, born_location, games, starter, minutes, points, rebounds, assists, blocks, steals) VALUES ((SELECT pa.name FROM player_attributes as pa WHERE pa.player_id = ?), (SELECT ta.abbreviation FROM player_attributes as pa, team_attributes as ta WHERE ta.team_id = pa.team_id AND ta.season = ? AND pa.player_id = ?), (SELECT pa.draft_year FROM player_attributes as pa WHERE pa.player_id = ?), (SELECT pa.born_location FROM player_attributes as pa WHERE pa.player_id = ?), (SELECT COUNT(*) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT SUM(starter) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT AVG(ps.minutes) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT AVG(ps.points) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT AVG(ps.offensive_rebounds + ps.defensive_rebounds) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT AVG(ps.assists) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT AVG(ps.blocks) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT AVG(ps.steals) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id))', (player_id, common.SEASON, player_id, player_id, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id))

    # Cache maxima
    g.db.execute('CREATE TEMPORARY TABLE %s_awards_max (name TEXT, abbreviation TEXT, draft_year INTEGER, born_location TEXT, games INTEGER, starter INTEGER, minutes FLOAT, points FLOAT, rebounds FLOAT, assists FLOAT, blocks FLOAT, steals FLOAT)', (g.league_id,))
    for player_id, in g.db.execute('SELECT player_id FROM player_attributes WHERE team_id >= 0'):
        g.db.execute('INSERT INTO awards_max (name, abbreviation, draft_year, born_location, games, starter, minutes, points, rebounds, assists, blocks, steals) VALUES ((SELECT pa.name FROM player_attributes as pa WHERE pa.player_id = ?), (SELECT ta.abbreviation FROM player_attributes as pa, team_attributes as ta WHERE ta.team_id = pa.team_id AND ta.season = ? AND pa.player_id = ?), (SELECT pa.draft_year FROM player_attributes as pa WHERE pa.player_id = ?), (SELECT pa.born_location FROM player_attributes as pa WHERE pa.player_id = ?), (SELECT COUNT(*) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT SUM(starter) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT MAX(ps.minutes) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT MAX(ps.points) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT MAX(ps.offensive_rebounds + ps.defensive_rebounds) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT MAX(ps.assists) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT MAX(ps.blocks) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id), (SELECT MAX(ps.steals) FROM player_attributes as pa, player_stats as ps WHERE pa.player_id = ps.player_id AND ps.season = ? AND pa.player_id = ? GROUP BY ps.player_id))', (player_id, common.SEASON, player_id, player_id, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id, common.SEASON, player_id))

    best_record_0 = g.db.execute('SELECT region || " " || name, won, lost FROM team_attributes WHERE season = ? AND (SELECT conference_id FROM league_divisions WHERE league_divisions.division_id = team_attributes.division_id) = 0 ORDER BY 1.0*won/(won + lost) DESC', (common.SEASON,)).fetchone()
    best_record_1 = g.db.execute('SELECT region || " " || name, won, lost FROM team_attributes WHERE season = ? AND (SELECT conference_id FROM league_divisions WHERE league_divisions.division_id = team_attributes.division_id) = 1 ORDER BY 1.0*won/(won + lost) DESC', (common.SEASON,)).fetchone()

    smoy = g.db.execute('SELECT name, abbreviation, minutes, points, rebounds, assists FROM awards_avg WHERE starter < 42 ORDER BY (0.75 * points) + assists + rebounds DESC').fetchone()
    mvp = g.db.execute('SELECT name, abbreviation, points, rebounds, assists FROM awards_avg ORDER BY (0.75 * points) + assists + rebounds DESC').fetchone()
    roy = g.db.execute('SELECT name, abbreviation, points, rebounds, assists FROM awards_avg WHERE draft_year = ? - 1 ORDER BY (0.75 * points) + assists + rebounds DESC', (common.SEASON,)).fetchone()
    dpoy = g.db.execute('SELECT name, abbreviation, rebounds, blocks, steals FROM awards_avg ORDER BY rebounds + 5 * blocks + 5 * steals DESC').fetchone()
    ipoy = g.db.execute('SELECT name, abbreviation, born_location, points, rebounds, assists FROM awards_avg WHERE born_location is not "USA" ORDER BY (0.75 * points) + assists + rebounds DESC').fetchone()
    firstteam = g.db.execute('SELECT name, abbreviation FROM awards_avg ORDER BY (0.75 * points) + assists + rebounds DESC').fetchmany(15)
    defteam = g.db.execute('SELECT name, abbreviation FROM awards_avg ORDER BY rebounds + 5 * blocks + 5 * steals DESC').fetchmany(15)
    most_points = g.db.execute('SELECT name, abbreviation, points FROM awards_avg ORDER BY points DESC').fetchone()
    highest_points = g.db.execute('SELECT name, abbreviation, points FROM awards_max ORDER BY points DESC').fetchone()
    most_rebounds = g.db.execute('SELECT name, abbreviation, rebounds FROM awards_avg ORDER BY rebounds DESC').fetchone()
    highest_rebounds = g.db.execute('SELECT name, abbreviation, rebounds FROM awards_max ORDER BY rebounds DESC').fetchone()
    most_assists = g.db.execute('SELECT name, abbreviation, assists FROM awards_avg ORDER BY assists DESC').fetchone()
    highest_assists = g.db.execute('SELECT name, abbreviation, assists FROM awards_max ORDER BY assists DESC').fetchone()
    most_steals = g.db.execute('SELECT name, abbreviation, steals FROM awards_avg ORDER BY steals DESC').fetchone()
    highest_steals = g.db.execute('SELECT name, abbreviation, steals FROM awards_max ORDER BY steals DESC').fetchone()
    most_blocks = g.db.execute('SELECT name, abbreviation, blocks FROM awards_avg ORDER BY blocks DESC').fetchone()
    highest_blocks = g.db.execute('SELECT name, abbreviation, blocks FROM awards_max ORDER BY blocks DESC').fetchone()

    g.db.execute('DROP TABLE awards_avg')
    g.db.execute('DROP TABLE awards_max')
