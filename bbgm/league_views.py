...
def box_score(gid=0):
    ...
    for row in r.fetchall():
        ...
        r = g.dbex('SELECT pa.pid, name, position, min, fg, fga, tp, tpa, ft, fta, orb, drb, orb + drb AS rebounds, ast, tov, stl, blk, pf, points FROM player_attributes as pa, player_stats as ps WHERE pa.pid = ps.pid AND ps.gid = :gid AND pa.tid = :tid ORDER BY starter DESC, min DESC', gid=gid, tid=teams[-1]['tid'])
