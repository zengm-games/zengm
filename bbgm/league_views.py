...
def schedule():
    ...
    for game in schedule_:
        ...
        if g.user_tid in tids:
            games.append([])
            for tid in tids:
                r = g.dbex('SELECT tid, abbreviation, region, name FROM team_attributes WHERE tid = :tid', tid=tid)
                row = r.fetchone()
                games[-1].append(row)
...
def standings(view_season=None):
    ...
    conferences = []
    r = g.dbex('SELECT cid, name FROM league_conferences ORDER BY cid ASC')
    for cid, conference_name in r.fetchall():
        conferences.append({'id': cid, 'name': conference_name, 'divisions': [], 'standings': ''})

        r = g.dbex('SELECT ld.did FROM league_divisions as ld WHERE ld.cid = :cid', cid=cid)
        ...
        r = g.dbex('SELECT did, name FROM league_divisions WHERE cid = :cid ORDER BY name ASC', cid=cid)
...
def playoffs(view_season=None):
    ...
    if view_season == g.season and g.phase < c.PHASE_PLAYOFFS:
        for cid in range(2):
            teams = []
            r = g.dbex('SELECT ta.name FROM team_attributes as ta, league_divisions as ld WHERE ld.did = ta.did AND ld.cid = :cid AND ta.season = :season ORDER BY 1.0*ta.won/(ta.won + ta.lost) DESC LIMIT 8', cid=cid, season=g.season)
...
@league_crap
def finances():
    r = g.dbex('SELECT ta.tid, ta.region, ta.name, ta.abbreviation, AVG(ts.attendance) AS attendance, SUM(ts.attendance)*:ticket_price / 1000000 AS revenue, (SUM(ts.attendance)*:ticket_price - SUM(ts.cost)) / 1000000 AS profit, ta.cash / 1000000 as cash, ((SELECT SUM(contract_amount) FROM player_attributes as pa WHERE pa.tid = ta.tid) + (SELECT IFNULL(SUM(contract_amount),0) FROM released_players_salaries as rps WHERE rps.tid = ta.tid)) / 1000 AS payroll FROM team_attributes as ta LEFT OUTER JOIN team_stats as ts ON ta.season = ts.season AND ta.tid = ts.tid WHERE ta.season = :season GROUP BY ta.tid', ticket_price=g.ticket_price, season=g.season)
...
def free_agents():
    ...
    r = g.dbex('SELECT pa.pid, pa.name, pa.position, :season - pa.born_date as age, pr.overall, pr.potential, AVG(ps.minutes) as minutes, AVG(ps.points) as points, AVG(ps.orb + ps.drb) as rebounds, AVG(ps.assists) as assists, pa.contract_amount/1000.0*(1+pa.free_agent_times_asked/10) as contract_amount, pa.contract_expiration FROM player_attributes as pa LEFT OUTER JOIN player_ratings as pr ON pr.season = :season AND pa.pid = pr.pid LEFT OUTER JOIN player_stats as ps ON ps.season = :season AND ps.is_playoffs = 0 AND pa.pid = ps.pid WHERE pa.tid = :tid GROUP BY pa.pid', season=g.season, tid=c.PLAYER_FREE_AGENT)
...
def trade_():
    ...
    if abbreviation is not None:
        new_tid_other, abbreviation = validate_abbreviation(abbreviation)
    else:
        new_tid_other = None
    ...
    r = g.dbex('SELECT pa.pid, pa.name, pa.position, :season - pa.born_date AS age, pr.overall, pr.potential, pa.contract_amount / 1000 AS contract_amount, pa.contract_expiration, AVG(ps.minutes) AS min, AVG(ps.points) AS pts, AVG(ps.orb + ps.drb) AS reb, AVG(ps.assists) AS ast FROM player_attributes AS pa LEFT OUTER JOIN player_ratings AS pr ON pr.season = :season AND pa.pid = pr.pid LEFT OUTER JOIN player_stats AS ps ON ps.season = :season AND ps.is_playoffs = 0 AND pa.pid = ps.pid WHERE pa.tid = :tid GROUP BY pa.pid ORDER BY pa.roster_position ASC', season=g.season, tid=g.user_tid)
    roster_user = r.fetchall()

    r = g.dbex('SELECT pa.pid, pa.name, pa.position, :season - pa.born_date AS age, pr.overall, pr.potential, pa.contract_amount / 1000 AS contract_amount, pa.contract_expiration, AVG(ps.minutes) AS min, AVG(ps.points) AS pts, AVG(ps.orb + ps.drb) AS reb, AVG(ps.assists) AS ast FROM player_attributes AS pa LEFT OUTER JOIN player_ratings AS pr ON pr.season = :season AND pa.pid = pr.pid LEFT OUTER JOIN player_stats AS ps ON ps.season = :season AND ps.is_playoffs = 0 AND pa.pid = ps.pid WHERE pa.tid = :tid GROUP BY pa.pid ORDER BY pa.roster_position ASC', season=g.season, tid=tid_other)
    roster_other = r.fetchall()

    summary = trade.summary(tid_other, pids_user, pids_other)

    r = g.dbex('SELECT tid, abbreviation, region, name FROM team_attributes WHERE season = :season AND tid != :tid ORDER BY tid ASC', season=g.season, tid=g.user_tid)
    teams = r.fetchall()
...
def draft_(view_season=None):
    ...
    if g.phase == c.PHASE_DRAFT and view_season == g.season:
        r = g.dbex('SELECT pa.pid, pa.position, pa.name, :season - pa.born_date as age, pr.overall, pr.potential FROM player_attributes as pa, player_ratings as pr WHERE pa.pid = pr.pid AND pa.tid = :tid AND pr.season = :season ORDER BY pr.overall + 2*pr.potential DESC', season=g.season, tid=c.PLAYER_UNDRAFTED)
        undrafted = r.fetchall()

        r = g.dbex('SELECT draft_round, pick, abbreviation, pid, name, :season - born_date as age, position, overall, potential FROM draft_results WHERE season = :season ORDER BY draft_round, pick ASC', season=g.season)
        drafted = r.fetchall()

        return render_all_or_json('draft.html', {'undrafted': undrafted, 'drafted': drafted})

    # Show a summary of an old draft
    r = g.dbex('SELECT dr.draft_round, dr.pick, dr.abbreviation, dr.pid, dr.name, :view_season - dr.born_date AS age, dr.position, dr.overall, dr.potential, ta.abbreviation AS current_abbreviation, :season - dr.born_date AS current_age, pr.overall AS current_overall, pr.potential AS current_potential, SUM(ps.minutes>0) AS gp, AVG(ps.minutes) as mpg, AVG(ps.points) AS ppg, AVG(ps.orb + ps.drb) AS rpg, AVG(ps.assists) AS apg FROM draft_results AS dr LEFT OUTER JOIN player_ratings AS pr ON pr.season = :season AND dr.pid = pr.pid LEFT OUTER JOIN player_stats AS ps ON ps.is_playoffs = 0 AND dr.pid = ps.pid LEFT OUTER JOIN player_attributes AS pa ON dr.pid = pa.pid LEFT OUTER JOIN team_attributes AS ta ON pa.tid = ta.tid AND ta.season = :season WHERE dr.season = :view_season GROUP BY dr.pid', view_season=view_season, season=g.season)
    players = r.fetchall()
...
def history(view_season=None):
    ...
    r = g.dbex('SELECT bre_tid, bre_abbreviation, bre_region, bre_name, bre_won, bre_lost, brw_tid, brw_abbreviation, brw_region, brw_name, brw_won, brw_lost, mvp_pid, mvp_name, mvp_tid, mvp_abbreviation, mvp_ppg, mvp_rpg, mvp_apg, dpoy_pid, dpoy_name, dpoy_tid, dpoy_abbreviation, dpoy_rpg, dpoy_bpg, dpoy_spg, smoy_pid, smoy_name, smoy_tid, smoy_abbreviation, smoy_ppg, smoy_rpg, smoy_apg, roy_pid, roy_name, roy_tid, roy_abbreviation, roy_ppg, roy_rpg, roy_apg FROM awards WHERE season = :season', season=view_season)
    if r.rowcount == 0:
        error = "You have to play through a season before there is any league history to view."
        return render_all_or_json('league_error.html', {'error': error})
    awards = r.fetchone()

    r = g.dbex('SELECT pid, name, abbreviation, ppg, rpg, apg, bpg, spg FROM awards_all_league WHERE season = :season AND team_type = \'league\' ORDER BY player_rank', season=view_season)
    all_league = r.fetchall()

    r = g.dbex('SELECT pid, name, abbreviation, ppg, rpg, apg, bpg, spg FROM awards_all_league WHERE season = :season AND team_type = \'defensive\' ORDER BY player_rank', season=view_season)
...
def roster(abbreviation=None, view_season=None):
    tid, abbreviation = validate_abbreviation(abbreviation)
    view_season = validate_season(view_season)
    seasons = get_seasons()
    ...
    if view_season == g.season:
        ...
        if tid == g.user_tid:
            r = g.dbex('SELECT COUNT(*) FROM team_stats WHERE tid = :tid AND season = :season AND is_playoffs = 0', tid=g.user_tid, season=g.season)
            n_games_remaining, = r.fetchone()
        else:
            n_games_remaining = 0  # Dummy value because only players on the user's team can be bought out
        r = g.dbex('SELECT pa.pid, pa.name, pa.position, :season - pa.born_date as age, pr.overall, pr.potential, pa.contract_amount / 1000 as contract_amount, pa.contract_expiration, AVG(ps.minutes) as minutes, AVG(ps.points) as points, AVG(ps.orb + ps.drb) as rebounds, AVG(ps.assists) as assists, ((1 + pa.contract_expiration - :season) * pa.contract_amount - :n_games_remaining / 82 * pa.contract_amount) / 1000 AS cash_owed FROM player_attributes as pa LEFT OUTER JOIN player_ratings as pr ON pr.season = :season AND pa.pid = pr.pid LEFT OUTER JOIN player_stats as ps ON ps.season = :season AND ps.is_playoffs = 0 AND pa.pid = ps.pid WHERE pa.tid = :tid GROUP BY pa.pid ORDER BY pa.roster_position ASC', season=view_season, n_games_remaining=n_games_remaining, tid=tid)
    else:
        # Only show players with stats, as that's where the team history is recorded
        r = g.dbex('SELECT pa.pid, pa.name, pa.position, :season - pa.born_date as age, pr.overall, pr.potential, pa.contract_amount / 1000 as contract_amount,  pa.contract_expiration, AVG(ps.minutes) as minutes, AVG(ps.points) as points, AVG(ps.orb + ps.drb) as rebounds, AVG(ps.assists) as assists FROM player_attributes as pa LEFT OUTER JOIN player_ratings as pr ON pr.season = :season AND pa.pid = pr.pid LEFT OUTER JOIN player_stats as ps ON ps.season = :season AND ps.is_playoffs = 0 AND pa.pid = ps.pid WHERE ps.tid = :tid GROUP BY pa.pid ORDER BY pa.roster_position ASC', season=view_season, tid=tid)
    players = r.fetchall()

    r = g.dbex('SELECT tid, abbreviation, region, name FROM team_attributes WHERE season = :season ORDER BY tid ASC', season=view_season)
    teams = r.fetchall()
...
def game_log(view_season=None, abbreviation=None):
    ...
    return render_all_or_json('game_log.html', {'abbreviation': abbreviation, 'teams': teams, 'tid': tid, 'seasons': seasons, 'view_season': view_season})
...
def negotiation_list(pid=None):
    ...
    r = g.dbex('SELECT pa.pid, pa.name, pa.position, :season - pa.born_date as age, pr.overall, pr.potential, AVG(ps.minutes) as minutes, AVG(ps.points) as points, AVG(ps.orb + ps.drb) as rebounds, AVG(ps.assists) as assists, pa.contract_amount/1000.0*(1+pa.free_agent_times_asked/10) as contract_amount, pa.contract_expiration FROM player_attributes as pa LEFT OUTER JOIN negotiation as n ON pa.pid = n.pid LEFT OUTER JOIN player_ratings as pr ON pr.season = :season AND pa.pid = pr.pid LEFT OUTER JOIN player_stats as ps ON ps.season = :season AND ps.is_playoffs = 0 AND pa.pid = ps.pid WHERE pa.tid = :tid AND n.resigning = 1 GROUP BY pa.pid', season=g.season, tid=c.PLAYER_FREE_AGENT)
...
def box_score(gid=0):
    ...
    for row in r.fetchall():
        ...
        r = g.dbex('SELECT pa.pid, name, position, minutes, fg, fga, tp, tpa, ft, fta, orb, drb, orb + drb AS rebounds, assists, turnovers, steals, blocks, pf, points FROM player_attributes as pa, player_stats as ps WHERE pa.pid = ps.pid AND ps.gid = :gid AND pa.tid = :tid ORDER BY starter DESC, minutes DESC', gid=gid, tid=teams[-1]['tid'])
...
def game_log_list():
    ...
    r = g.dbex('SELECT gid, home, (SELECT abbreviation FROM team_attributes WHERE tid = opp_tid AND season = :season) as opponent_abbreviation, won, points, opp_pts FROM team_stats WHERE tid = :tid AND season = :season', season=season, tid=tid)
    games = r.fetchall()
...
def validate_abbreviation(abbreviation):
    ...
    if abbreviation:
        r = g.dbex('SELECT tid FROM team_attributes WHERE season = :season AND abbreviation = :abbreviation', season=g.season, abbreviation=abbreviation)
        if r.rowcount == 1:
            tid, = r.fetchone()
    ...
    if not tid:
        tid = g.user_tid
        r = g.dbex('SELECT abbreviation FROM team_attributes WHERE season = :season AND tid = :tid', season=g.season, tid=tid)
        abbreviation, = r.fetchone()

    return tid, abbreviation
