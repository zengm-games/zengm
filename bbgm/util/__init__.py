...
def get_payroll(team_id):
    ...
    """
    g.db.execute('SELECT SUM(contract_amount) FROM player_attributes WHERE team_id = %s AND contract_expiration >= %s', (team_id, g.season))
    payroll, = g.db.fetchone()

    g.db.execute('SELECT SUM(contract_amount) FROM released_players_salaries WHERE team_id = %s', (team_id,))
...
def free_agents_auto_sign():
    ...
    free_agents = []
    g.db.execute('SELECT pa.player_id, pa.contract_amount, pa.contract_expiration FROM player_attributes as pa, player_ratings as pr WHERE pa.team_id = %s AND pa.player_id = pr.player_id AND pr.season = %s ORDER BY pr.overall + 2*pr.potential DESC', (g.season, c.PLAYER_FREE_AGENT))
    ...
    for i in xrange(30):
        ...
        g.db.execute('SELECT count(*) FROM player_attributes WHERE team_id = %s', (team_id,))
        ...
        while payroll < g.salary_cap and num_players < 15:
            ...
            for player_id, amount, expiration, signed in free_agents:
                if amount + payroll <= g.salary_cap and not signed and num_players < 15:
                    g.db.execute('UPDATE player_attributes SET team_id = %s, contract_amount = %s, contract_expiration = %s WHERE player_id = %s', (team_id, amount, expiration, player_id))
...
def free_agents_decrease_demands():
    ...
    g.db.execute('SELECT player_id, contract_amount, contract_expiration FROM player_attributes WHERE team_id = %s AND contract_amount > 500', (c.PLAYER_FREE_AGENT,))
    for player_id, amount, expiration in g.db.fetchall():
        amount -= 50
        if amount < 500:
            amount = 500
        ...
        if amount < 1000:
            expiration = g.season
        g.db.execute('UPDATE player_attributes SET contract_amount = %s, contract_expiration = %s WHERE player_id = %s', (amount, expiration, player_id))
    ...
    g.db.execute('UPDATE player_attributes SET free_agent_times_asked = free_agent_times_asked - 0.1 WHERE team_id = %s', (c.PLAYER_FREE_AGENT,))
    g.db.execute('UPDATE player_attributes SET free_agent_times_asked = 0 WHERE team_id = %s AND free_agent_times_asked < 0', (c.PLAYER_FREE_AGENT,))
