...
class SeasonEndWindow:
    ...
    def __init__(self, main_window):
        ...
        best_record_0 = common.DB_CON.execute('SELECT region || " " || name, won, lost FROM team_attributes WHERE season = ? AND (SELECT conference_id FROM league_divisions WHERE league_divisions.division_id = team_attributes.division_id) = 0 ORDER BY 1.0*won/(won + lost) DESC', (common.SEASON,)).fetchone()
        best_record_1 = common.DB_CON.execute('SELECT region || " " || name, won, lost FROM team_attributes WHERE season = ? AND (SELECT conference_id FROM league_divisions WHERE league_divisions.division_id = team_attributes.division_id) = 1 ORDER BY 1.0*won/(won + lost) DESC', (common.SEASON,)).fetchone()
