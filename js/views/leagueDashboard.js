...
define(["db", "globals", "ui", "core/season", "lib/jquery", "lib/knockout", "lib/knockout.mapping", "lib/underscore", "util/helpers", "util/viewHelpers"], function (db, g, ui, season, $, ko, mapping, _, helpers, viewHelpers) {
    ...
    function loadBefore(updateEvents, cb) {
        var getGames, getPayroll, getPlayers, getPlayoffs, getSchedule, getTeam, getTeams, tx, vars;
        ...
        getTeam = function (updateEvents) {
            var deferred;

            deferred = $.Deferred();

            if (vm.firstRun || updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0 || (updateEvents.indexOf("newPhase") >= 0 && g.phase === g.PHASE.PRESEASON)) {
        ...
        getPayroll = function (updateEvents) {
            var deferred;

            deferred = $.Deferred();

            if (vm.firstRun || updateEvents.indexOf("playerMovement") >= 0) {
        ...
        getTeams = function (updateEvents) {
            var attributes, deferred, seasonAttributes, stats;
        ...
        getGames = function (updateEvents) {
            var deferred;

            deferred = $.Deferred();

            if (vm.firstRun || updateEvents.indexOf("gameSim") >= 0 || (updateEvents.indexOf("newPhase") >= 0 && g.phase === g.PHASE.PRESEASON)) {
        ...
        getSchedule = function (updateEvents) {
            var deferred;
        ...
        getPlayers = function (updateEvents) {
            var deferred;
        ...
        getPlayoffs = function (updateEvents) {
            var deferred;
            ...
            if (vm.firstRun || (g.phase >= g.PHASE.PLAYOFFS && updateEvents.indexOf("gameSim") >= 0) || (updateEvents.indexOf("newPhase") >= 0 && g.phase === g.PHASE.PLAYOFFS)) {
        ...
        $.when(getTeam(updateEvents), getPayroll(updateEvents), getTeams(updateEvents), getGames(updateEvents), getSchedule(updateEvents), getPlayers(updateEvents), getPlayoffs(updateEvents)).then(function () {
