        ...
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || !vm.retired()) {
            g.dbl.transaction("players").objectStore("players").get(inputs.pid).onsuccess = function (event) {
                ...
                p = player.filter(event.target.result, {
                    attrs: ["pid", "name", "tid", "abbrev", "teamRegion", "teamName", "pos", "age", "hgtFt", "hgtIn", "weight", "born", "contract", "draft", "face", "mood", "injury", "salaries", "salariesTotal", "awards", "freeAgentMood", "imgURL"],
    ...
    function uiFirst(vm) {
        ...
        ko.computed(function () {
            ...
            if (vm.player.imgURL && vm.player.imgURL()) {
                ...
                pic.innerHTML = '<img src="' + vm.player.imgURL() + '" style="margin-top: 12px; width: 100px"></img>';
            } else {
