...
// Resolve the formula table once for a batch of players/stat rows.
export const compileStats = (
	...
	const keys = [...stats];
	const statFunctions2 = statFunctions as Record<string, StatFunction>;
	const functions: (StatFunction | undefined)[] = [];
	for (const stat of keys) {
		const fn = statFunctions2[stat];
		if (fn && typeof fn !== "function") {
			// Preserve the original error behavior for unusual inherited keys.
			return (ps: PlayerStats, bornYear: number | undefined) =>
				processStats(ps, keys, bornYear, getFantasyPoints);
		}
		functions.push(fn ? fn.bind(statFunctions2) : undefined);
	}

	return (ps: PlayerStats, bornYear: number | undefined) => {
		const row: any = {};
		const extra = { bornYear, getFantasyPoints };
		for (let i = 0; i < keys.length; i++) {
			const stat = keys[i]!;
			const fn = functions[i];
			let value = fn ? fn(ps, extra) : ps[stat];
			if (
