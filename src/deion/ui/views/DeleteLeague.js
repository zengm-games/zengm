...
const DeleteLeague = ({ lid, name, numPlayers, numSeasons }) => {
	...
	if (name !== undefined) {
		message = (
			<p>
				Are you <i>absolutely</i> sure you want to delete {name} (League {lid}
				)? You will <i>permanently</i> lose any record of all{" "}
				{helpers.numberWithCommas(numSeasons)} seasons and{" "}
				{helpers.numberWithCommas(numPlayers)} players from this league (well...
		...
		message = (
			<p>
				Are you <i>absolutely</i> sure you want to delete League {lid}? You will{" "}
				<i>permanently</i> lose any record of all seasons, players, and games
				from this league (well... unless you have a backup somewhere).
			</p>
		);
	}
