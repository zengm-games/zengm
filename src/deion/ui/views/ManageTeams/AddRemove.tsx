...
const AddRemove = ({ dispatch, confs, divs, phase, saving }: Props) => {
	...
	const addTeam = async (e: FormEvent) => {
		...
		const t = await toWorker("main", "addTeam", did);
		dispatch({
			type: "addTeam",
			team: t,
		});
		...
		<>
			...
			<div className="row">
				<div className="col-sm">
					<p>
						After you add a team, it will become available to edit in the form
						below. Created teams start with no players. You can use God Mode to
						set up the roster, or start simulating and the AI will sign free
						agents. There is not yet support for a real expansion draft.
					</p>
