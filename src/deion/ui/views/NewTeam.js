...
class NewTeam extends React.Component {
	constructor(props) {
		...
		this.state = {
			tid:
				props.teams && props.teams.length > 0 ? props.teams[0].tid : undefined,
		};
	...
	render() {
		...
		if (!gameOver && !godMode) {
			return (
				<div>
					<h1>Error</h1>
					<p>
						You may only switch to another team after you're fired or when
						you're in <a href={helpers.leagueUrl(["god_mode"])}>God Mode</a>
					</p>
				</div>
			);
		}
		...
		if (godMode) {
			message = (
				<p>
					Because you're in{" "}
					<a href={helpers.leagueUrl(["god_mode"])}>God Mode</a>, you can become
					the GM of any team.
				</p>
			);
		} else {
			message = (
				<p>
					After you were fired, your agent tried to get you job offers from
					other teams. Unfortunately, he was only able to secure offers from
					some of the worst teams in the league. Are you interested in running
					any of these teams?
				</p>
			);
		}
