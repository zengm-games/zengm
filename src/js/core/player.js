 ...
 * @param {boolean=} options.playoffs Boolean representing whether to return playoff stats (statsPlayoffs and careerStatsPlayoffs) or not; default is false. Either way, regular season stats are always returned. Note: player.filter is a *filtering* function. If playoff stats are not already included in the player objects (e.g. set `statsPlayoffs: true` in the options argument for `player.withStats()`), then the statsPlayoffs and careerStatsPlayoffs fields returned by this function will default to zero, since there's nothing to add up.
