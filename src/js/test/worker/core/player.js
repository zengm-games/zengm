...
describe("core/player", () => {
    describe("#generate()", () => {
        it.skip("should add stats row only for players generated on teams, not free agents or undrafted players", () => {
            ...
            let p = player.generate(-2, 19, 25, 2012, false, 15.5);
            assert.equal(p.stats.length, 0);

            p = player.generate(-1, 19, 25, 2012, false, 15.5);
            assert.equal(p.stats.length, 0);

            p = player.generate(0, 19, 25, 2012, false, 15.5);
            assert.equal(p.stats.length, 1);

            p = player.generate(15, 19, 25, 2012, false, 15.5);
            assert.equal(p.stats.length, 1);
        });
    });
    ...
    describe("#madeHof()", () => {
        it("should correctly assign players to the Hall of Fame", () => {
            ...
            const p = player.generate(0, 19, 25, 2012, false, 15.5);
            const playerStats = [
                {
                    min: 1 * 2.6,
                    per: 18.7,
    ...
    describe("#getPlayerFakeAge()", () => {
        it("should pick appropriate player to have a fake age", async () => {
            ...
            const players = [
                player.generate(0, 19, 25, g.season, false, 15.5),
                player.generate(0, 25, 25, g.season, false, 15.5), // Should get filtered out, too old
                player.generate(0, 22, 25, g.season, false, 15.5),
                player.generate(0, 20, 25, g.season, false, 15.5),
            ];
            players[0].pid = 0;
