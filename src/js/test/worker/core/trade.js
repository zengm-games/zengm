...
describe("core/trade", () => {
    ...
    describe("#create()", () => {
        ...
        it("should create trade with player ID", async () => {
            await trade.create([{tid: g.userTid, pids: [], dpids: []}, {tid: 2, pids: [2], dpids: []}]);
    ...
    describe("#updatePlayers()", () => {
        it("should allow players from both teams to be set", async () => {
            ...
            const teams = await trade.updatePlayers([{tid: g.userTid, pids: userPidsTest, dpids: []}, {tid: 1, pids: otherPidsTest, dpids: []}]);
            assert.deepEqual(teams[0].pids, userPidsTest);
            assert.deepEqual(teams[1].pids, otherPidsTest);
        });
        it("should filter out invalid players", async () => {
            await trade.create([{tid: g.userTid, pids: [], dpids: []}, {tid: 1, pids: [], dpids: []}]);
            const teams = await trade.updatePlayers([{tid: g.userTid, pids: [1, 16, 20, 48, 50, 90], dpids: []}, {tid: 1, pids: [12, 63, 3, 87, 97, 524], dpids: []}]);
        ...
        it("should delete the other team's players, but not the user's players, from the trade when a new team is selected", async () => {
            ...
            const userPidsTest = [0, 1];
            const otherPidsTest = [2, 3];
            let teams = await trade.updatePlayers([{tid: g.userTid, pids: userPidsTest, dpids: []}, {tid: 1, pids: otherPidsTest, dpids: []}]);
            assert.deepEqual(teams[0].pids, userPidsTest);
            assert.deepEqual(teams[1].pids, otherPidsTest);

            await trade.create([{tid: g.userTid, pids: [], dpids: []}, {tid: 2, pids: [], dpids: []}]);
