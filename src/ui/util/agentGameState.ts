...
export type AgentGameContext = {
	...
	statusText: string;
	/** tid + abbrev for every team; used to resolve getRoster(teamAbbrev) and trades. */
	teamsIndex: {
		tid: number;
		abbrev: string;
	}[];
...
export const serializeAgentGameState = (
	...
	const teamsIndex: AgentGameContext["teamsIndex"] = [];
	for (let tid = 0; tid < state.teamInfoCache.length; tid++) {
		const info = state.teamInfoCache[tid];
		if (info?.abbrev) {
			teamsIndex.push({ tid, abbrev: info.abbrev });
		}
	}
