...
// ============================================
// Direct Firestore operations from UI thread
// (Worker has issues with Firebase in SharedWorker context)
// ============================================

export type CloudLeagueData = {
	cloudId: string;
	name: string;
	sport: string;
	ownerId: string;
...
/**
 * Create a new cloud league document in Firestore (UI thread)
 */
export const createCloudLeagueDoc = async (
	name: string,
	sport: string,
	userId: string,
): Promise<string> => {
	console.log("[Firebase UI] createCloudLeagueDoc started", { name, sport, userId });

	const firestore = getFirebaseDb();
	const cloudId = `league-${Date.now()}-${Math.random().toString(36).slice(2)}`;

	const leagueData: CloudLeagueData = {
		cloudId,
		name,
		sport,
		ownerId: userId,
	...
	const leagueRef = doc(firestore, "leagues", cloudId);
	console.log("[Firebase UI] Calling setDoc for league:", cloudId);

	await setDoc(leagueRef, leagueData);
	console.log("[Firebase UI] League created successfully:", cloudId);

	return cloudId;
};

/**
 * Upload data to a cloud league store (UI thread)
 */
export const uploadToCloudStore = async (
	cloudId: string,
	storeName: string,
	records: any[],
	...
	const pk = pkMap[storeName] || "id";
	const collectionPath = `leagues/${cloudId}/${storeName}`;
	...
	for (let i = 0; i < records.length; i++) {
		...
		if (batchCount > 0 && (batchCount >= BATCH_SIZE || batchSize + recordSize > MAX_PAYLOAD_SIZE)) {
			if (onProgress) {
				onProgress(`Uploading ${storeName} batch ${batchNum}/${totalBatches}...`);
			}
			await batch.commit();
	...
	if (batchCount > 0) {
		if (onProgress) {
			onProgress(`Uploading ${storeName} (final batch)...`);
		}
		await batch.commit();
	}
};
