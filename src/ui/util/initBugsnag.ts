...
Bugsnag.start({
	...
	onError: (event) => {
		...
		if (event && typeof event.context === "string") {
			...
			// All the errors in my application code include "src/" in the filename of some source code file in the stack trace. So hopefully this will filter out all the errors from ads.
			const isInApplicationCode = event.errors.some((error) =>
				error.stacktrace.some((row) => row.file.includes("src/")),
			);
