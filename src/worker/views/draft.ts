...
const updateDraft = async (inputs: unknown, updateEvents: UpdateEvents) => {
	if (
		...
		const config: TableConfig = new TableConfig("draft", [
			"Name",
			"Pos",
			"Age",
			"Ovr",
		...
		const draftPicks: any[] = await getDraftPicks();

		const drafted: any[] = await idb.getCopies.playersPlus(
			await getDraftedPlayers(),
			{
				attrs: ["draft", ...config.attrsNeeded],
		...
		const undrafted: any[] = (
			await idb.getCopies.playersPlus(await getUndraftedPlayers(draftPicks), {
				attrs: ["draft", "valueFuzz", ...config.attrsNeeded],
				ratings: config.ratingsNeeded,
				stats: config.statsNeeded,
				season: g.get("season"),
				showRookies: true,
...
const expansionDraft = (): GameAttributesLeague["expansionDraft"] =>
	g.get("expansionDraft");
...
const getUndraftedPlayers = async (draftPicks: any[]) => {
	let undrafted: any[];
