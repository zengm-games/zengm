...
const getName = (untrimmedName, file) => {
    ...
    if (parts.length !== 2) {
        ...
        const middlesWithLast = [['De', 'La'], ['Van', 'De'], ['Van', 'Den'], ['Van', 'Der'], ['Da'], ['De'], ['Del'], ['Della'], ['Di'], ['Dos'], ['La'], ['Le'], ['Li'], ['San'], ['Santa'], ['St.'], ['Van'], ['Vander'], ['Von']];
        let stop = false;
        ...
        if (parts.length > 3 && (parts[parts.length - 2] === 'A' || parts[parts.length - 2] === 'a')) {
            parts = [parts[0], parts.slice(parts.length - 3, parts.length).join(' ')];
        }
        ...
        if (parts.length !== 2) {
            console.log(parts);
            throw new Error(`Unable to parse first name and last name from ${name} in ${file}`);
        }
    }
...
const getCountry = (misc, file) => {
    ...
    country = countryFixes.hasOwnProperty(country) ? countryFixes[country] : country;
...
for (const country of Object.keys(fnsByCountry).sort()) {
    countsByCountry[country] = Math.min(Object.keys(fnsByCountry[country]).length, Object.keys(lnsByCountry[country]).length);
}
...
const namesByCountryCumsum = namesByCountry => {
    ...
    for (const country of Object.keys(namesByCountry).sort()) {
        let cumsum = 0;
        obj[country] = Object.keys(namesByCountry[country]).sort().map(name => {
